...
export function PromptInputAttachment({
  ...
  const safeFilename = filename === '' ? undefined : filename;
  const mediaType =
    ...
    >
      <div className="space-y-2 py-1" ref={contentRef}>
        <div className="flex flex-wrap gap-2">
          {attachments.files
            .filter(
              (f) => !(f.mediaType.startsWith('image/') && Boolean(f.url))
            )
            .map((file) => (
        ...
        <div className="flex flex-wrap gap-2">
          {attachments.files
            .filter((f) => f.mediaType.startsWith('image/') && Boolean(f.url))
            .map((file) => (
              <Fragment key={file.id}>{children(file)}</Fragment>
            ))}
        </div>
...
export const PromptInputSpeechButton = ({
  ...
  useEffect(() => {
    ...
    const handleResult = (event: Event) => {
      ...
      for (const result of Array.from(recognitionEvent.results)) {
        if (result.isFinal) {
